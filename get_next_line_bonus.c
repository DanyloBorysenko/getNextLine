...
/*   Updated: 2025/11/24 10:53:25 by danborys         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*create_rem(char *leavings[], int fd)
{
	char	*ptr;
...
{
	static char	*leavings[1024];
	char		*line;
	char		*del;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	create_rem(leavings, fd);
	if (!leavings[fd])
		return (NULL);
	leavings[fd] = read_fd(fd, leavings[fd]);
	if (!leavings[fd])
		return (NULL);
	if (*leavings[fd] == '\0')
	{
	...
	del = ft_strchr(leavings[fd], '\n');
	line = extract_line(leavings[fd], del);
	leavings[fd] = trim_rem(leavings[fd], del);
	return (line);
}
// int main(void)
// {
